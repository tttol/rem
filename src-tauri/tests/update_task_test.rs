...
#[test]
fn update_status_from_todo_to_doing() {
    // Arrange
    let temp_dir = TempDir::new().expect("Failed to create temp directory");
    let app_data_dir = temp_dir.path().to_path_buf();
    
    let todo_dir = app_data_dir.join("todo");
    let doing_dir = app_data_dir.join("doing");
    fs::create_dir_all(&todo_dir).expect("Failed to create todo directory");
    fs::create_dir_all(&doing_dir).expect("Failed to create doing directory");
    
    let original_task = Task {
        id: "test_task_status".to_string(),
    ...
    let original_task_json = serde_json::to_string(&original_task).expect("Failed to serialize original task");
    let todo_file_path = todo_dir.join("test_task_status.json");
    let doing_file_path = doing_dir.join("test_task_status.json");
    fs::write(&todo_file_path, original_task_json).expect("Failed to write original task file");
    
    // Act
    let result = update_task::update_status(
        &app_data_dir,
        "test_task_status",
        "todo",
        "doing"
    ...
    // Assert
    assert!(result.is_ok(), "update_status should succeed");
    
    // Verify the file was moved from todo to doing
    assert!(!todo_file_path.exists(), "Original file in todo should be deleted");
    assert!(doing_file_path.exists(), "New file in doing should be created");
    
    let updated_content = fs::read_to_string(&doing_file_path).expect("Failed to read updated task file");
    let updated_task: Task = serde_json::from_str(&updated_content).expect("Failed to parse updated task");
    
    assert_eq!(updated_task.id, "test_task_status");
    assert_eq!(updated_task.title, "Task Title");
    assert_eq!(updated_task.status, "doing");
    assert_eq!(updated_task.description, "Task description");
}
