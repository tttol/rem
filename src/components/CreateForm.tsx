...
function CreateForm({setIsShowForm, reload}: {setIsShowForm: (isShowForm: boolean) => void, reload: () => void}) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-md mx-auto p-4 bg-gray-800 rounded-lg border border-gray-600">
      <p className="text-2xl text-white">Add a new task</p>
      <div className="mb-4">
        <input 
          id="title"
          type="text" 
          ...
          placeholder="title"
          autoCapitalize="off"
          className={`w-full px-3 py-2 border rounded-md bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:border-transparent ${errors.title ? 'border-red-500 focus:ring-red-500' : 'border-gray-500 focus:ring-blue-500'}`}
